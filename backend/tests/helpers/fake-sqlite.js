 ...
 */
export function createFakeSqlite({ total = 4 } = {}) {
  const prepareCalls = [];
  const execCalls = [];
  const statements = [];
  ...
  function makeStatement(sql) {
    ...
    // Conteo de siembra de db.js: por defecto la tabla está "no vacía" y no se
    // siembra nada. Pasar `{ total: 0 }` es la única forma de ejercitar la
    // rama de siembra desde un test.
    if (/COUNT\(\*\)/i.test(sql)) {
      stmt.get = jest.fn(() => ({ total }));
    }
